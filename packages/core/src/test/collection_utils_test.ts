...
describe('CollectionUtils', function () {
	describe('#createPath()', function () {
		...
		it('should check collectionGroup on a path on a second level collection', async () => {
			...
			).collectionGroup.should.eql('comments');
		});
	});
	...
	describe('#replacePathSegments()', function () {
		it('should replace path segments', async () => {
			...
			CollectionUtils.replacePathSegments('workoutPrograms/{programId}/workouts/{workoutId}', 'id1').should.eql(
				'workoutPrograms/id1/workouts'
			);
			CollectionUtils.replacePathSegments(
				'workoutPrograms/{programId}/workouts/{workoutId}',
				'id1',
				'id2'
			).should.eql('workoutPrograms/id1/workouts/id2');
		});
	});
});
