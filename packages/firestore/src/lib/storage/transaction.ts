...
export class FirestoreTransaction {
	...
	async find<T, Path extends CollectionPath<any, any, any>>(repo: BaseRepository<T, Path>, ids: DocumentIds<Path>): Promise<T | null> {
	...
	async get<T, Path extends CollectionPath<any, any, any>>(repo: BaseRepository<T, Path>, ids: DocumentIds<Path>): Promise<T> {
	...
	create<T, Path extends CollectionPath<any, any, any>>(repo: BaseRepository<T, Path>, data: T, ids: DocumentIds<Path>): FirestoreTransaction {
		return this.applyToDoc(repo, data, ids, (id, data, doc) => {
			this.transaction.create(doc, data);
		})
	};

	set<T, Path extends CollectionPath<any, any, any>>(repo: BaseRepository<T, Path>, data: T, ids: DocumentIds<Path>): FirestoreTransaction {
		return this.applyToDoc(repo, data, ids, (id, data, doc) => {
			this.transaction.set(doc, data, {merge: true});
		})
	};

	write<T, Path extends CollectionPath<any, any, any>>(repo: BaseRepository<T, Path>, data: T, ids: DocumentIds<Path>): FirestoreTransaction {
		return this.applyToDoc(repo, data, ids, (id, data, doc) => {
			this.transaction.set(doc, data, {merge: false});
		})
	};

	update<T, Path extends CollectionPath<any, any, any>>(repo: BaseRepository<T, Path>, data: T, ids: DocumentIds<Path>): FirestoreTransaction {
		return this.applyToDoc(repo, data, ids, (id, data, doc) => {
			this.transaction.update(doc, data, {exists: true});
		})
	};

	delete<T, Path extends CollectionPath<any, any, any>>(repo: BaseRepository<T, Path>, data: T, ids: DocumentIds<Path>): FirestoreTransaction {
		return this.applyToDoc(repo, data, ids, (id, data, doc) => {
